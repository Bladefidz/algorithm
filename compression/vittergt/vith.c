...
void update_treeVITTER ( huff_model *h, int c );
huffnode_t *get_block_leader_VITTER( huff_model *h, huffnode_t *node );
void slide_node_VITTER( huff_model *h, huffnode_t *node );
void SlideAndIncrement_VITTER( huff_model *h, huffnode_t **node );
void remove_symbol( huffnode_t *node );

void slide_node( huff_model *h, huffnode_t *node );
void SlideAndIncrement( huff_model *h, huffnode_t **node );
...
void slide_node_VITTER( huff_model *h, huffnode_t *node )
{
	...
	for ( i = node->number + 1; i < ROOT_NODE_NUMBER; i++ ) {
		...
		if ( freq >= h->numbers[i]->freq ){
			if ( h->numbers[i] != node->parent ){
				/*
				...
				*/
				if ( node->ch == INTERNAL_NODE
								&& h->numbers[i]->ch == INTERNAL_NODE
								&& freq == h->numbers[i]->freq )
					break;
				else swap_nodes( h, node, h->numbers[i] );	/* quick swap. */
			}
		}
		else break;
	}
...
void update_treeVITTER ( huff_model *h, int c )
{
	...
	*node = h->hufflist[ c ];        /* Vitter's "q" is "node" here. */
	...
	if ( node == NULL ) {	/* have we just received a new symbol? */
		...
		*/
		...
		leader = get_block_leader_VITTER( h, node );
		if ( leader ){
			swap_nodes( h, node, leader );
		}

		/* if the node is the sibling of the 0-node. */
		if ( node->parent == h->zero_node->parent ){
			increment = node;
			node = node->parent;
		}
	}
	...
	while( node != h->top ) {
		...
		SlideAndIncrement_VITTER( h, &node );
	}

	if ( increment != NULL ) {
		SlideAndIncrement_VITTER( h, &increment );
	}
}
...
void remove_symbol( huffnode_t *node )
{
	if ( node->parent->parent ) {
		if ( node == node->parent->child_1 )
				node = node->parent->child_2;
		else node = node->parent->child_1;

		if ( node->parent->parent &&
			node->parent->parent->child_1 == node->parent )
				node->parent->parent->child_1 = node;
		else node->parent->parent->child_2 = node;
	...
	for ( i = node->number + 1; i < ROOT_NODE_NUMBER; i++ ) {
		if ( node->freq == h->numbers[i]->freq ) {
			if ( h->numbers[i] != node->parent ){
				...
				swap_nodes( h, node, h->numbers[i] );	/* quick swap. */
			}
		}
		else break;
	}
...
void SlideAndIncrement( huff_model *h, huffnode_t **node )
{
	...
	if ( (*node)->ch == INTERNAL_NODE ){ /* internal node? */
		temp = (*node)->parent;
		slide_node( h, *node ); /* promote as leader of its block. */
	}
