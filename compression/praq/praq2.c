/*
	Filename:     praq2.c
	Description:  PPP/LZP with variable-length encoding of MTF-1 codes.
...
void usage( void )
{
	fprintf(stderr, "\n Usage: praq2 c[1|2]|d infile outfile\n"
		...
		while ( n < nread ) {
			if ( w[prev] == (c=p[n&WMASK]) ){
				...
				if ( mcode == mPPP ) {
					...
					/* symbol-frequency coding */
					put_vlcode( mtf(c), BSIZE );  /* c is moved to front here.. */
					/* but if 2nd position symbol is higher in frequency, let c 
