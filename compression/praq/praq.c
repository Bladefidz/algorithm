/*
	...
	Written by:   Gerald R. Tamayo, July 2009, (4/14/2010, 9/2/2017)
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* Bitsize of the first N high-ranking 
...
void usage( void )
{
	fprintf(stderr, "\n Usage: praq c[1|2]|d infile outfile\n"
...
void copyright( void )
{
	fprintf(stderr, "\n Written by: Gerald R. Tamayo, 2009\n");
}
...
void compress( unsigned char w[], unsigned char p[] )
{
	...
	while ( (nread=fread(p, 1, WSIZE, gIN)) ){
		...
		while ( n < nread ) {
			if ( w[prev] == (c=p[n&WMASK]) ){
				...
				b = 0;
				if ( mcode == mPPP ) put_nbits(c, 8);
				else put_vlcode( mtf(c), BSIZE );
	...
	if ( mcode == mPPP ) put_nbits(EOF_CODE, 8);
	else put_vlcode(mtf(EOF_CODE), BSIZE);
	put_ONE();
}
...
void decompress( unsigned char w[] )
{
	...
	do {
		...
		if ( mcode == mPPP ) c = get_nbits(8);
		else c = get_mtf_c(get_vlcode(BSIZE));
		if ( c == EOF_CODE && get_bit() ) return;
		pfputc( c );
		w[prev] = c;
		prev = ((prev<<5)+c) & WMASK;
	} while( 1 );
